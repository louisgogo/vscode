...
class budget():
    ...
    def down_loads(self, ReportType1, ReportType2):
        ...
        for i in range(self.num, self.end):
            ...
            self.driver.switch_to.frame("selEpList")
            # 选择单位
            if i >= 2 and self.adjust == False:
                elem = self.driver.find_element_by_id("treeLeft_%s_switch" % i)
                elem.click()
            elem = self.driver.find_element_by_id("treeLeft_%s_span" % i)
            elem.click()
            # 控制滚动轴
            target = self.driver.find_element_by_id("treeLeft_%s_span" % i)
            self.driver.execute_script(
